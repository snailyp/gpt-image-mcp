...
@pytest.mark.asyncio
async def test_edit_image_tool_file_format():
    """Test edit_image_tool with file output format."""
    from src.tools.edit import edit_image_tool
    ...
    with patch('src.tools.common.OpenAIClient') as mock_client_class, \
        ...
        assert result["success"] is True
        assert result["format"] == "file"
        assert "12345-edited.png" in result["data"]
        assert result["metadata"]["model"] == "gpt-4o"
        assert result["metadata"]["size"] == "1792x1024"
        assert result["metadata"]["quality"] == "hd"
...
@pytest.mark.asyncio
async def test_edit_image_tool_base64_format():
    """Test edit_image_tool with base64 output format."""
    from src.tools.edit import edit_image_tool
...
@pytest.mark.asyncio
async def test_edit_image_tool_custom_output_path():
    """Test edit_image_tool with custom output path."""
    from src.tools.edit import edit_image_tool
    ...
    with patch('src.tools.common.OpenAIClient') as mock_client_class, \
        ...
        mock_handler.download_image = AsyncMock(return_value={
            "format": "file",
            "data": "./custom/path/edited_image.png"
        })
        mock_handler_class.return_value = mock_handler
        ...
        result = await edit_image_tool(
            ...
            output_format="file",
            output_path="./custom/path/edited_image.png",
            api_key="test_api_key"
        )
        ...
        assert result["success"] is True
        assert result["format"] == "file"
        assert result["data"] == "./custom/path/edited_image.png"
...
@pytest.mark.asyncio
async def test_edit_image_tool_error_handling():
    """Test edit_image_tool error handling."""
    from src.tools.edit import edit_image_tool
    ...
    with patch('src.tools.common.OpenAIClient') as mock_client_class:
        ...
        # Setup mock to raise an exception
        mock_client = Mock()
        mock_client.edit_image = AsyncMock(side_effect=Exception("API connection failed"))
        mock_client_class.return_value = mock_client
...
@pytest.mark.asyncio
async def test_edit_image_tool_custom_base_url():
    """Test edit_image_tool with custom base URL."""
    from src.tools.edit import edit_image_tool
...
@pytest.mark.asyncio
async def test_edit_image_tool_empty_api_key():
    """Test edit_image_tool with empty API key."""
    from src.tools.edit import edit_image_tool
...
@pytest.mark.asyncio
async def test_edit_image_tool_empty_image_input():
    """Test edit_image_tool with empty image_input."""
    from src.tools.edit import edit_image_tool

    # Call the tool with empty image_input
    result = await edit_image_tool(
        image_input="",
        prompt="This should fail",
        api_key="test_api_key"
